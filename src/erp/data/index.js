...
/* --------------------------- derived KPIs ------------------------------ */
const totalContractValue = projects.reduce((s, p) => s + p.budget, 0);
...
const portfolioMargin =
  projects.reduce((s, p) => s + p.margin * p.budget, 0) / totalContractValue;

const outstanding = invoices
  .filter((i) => i.status !== "Paid")
  .reduce((s, i) => s + i.amount + i.gst, 0);

const collected = payments.reduce((s, p) => s + p.amount, 0);

const bestVendor = [...vendors].sort((a, b) => b.rating - a.rating)[0];
const bestEmployee = [...employees].sort((a, b) => b.productivity - a.productivity)[0];
...
const conversionRate = Math.round(
  (leads.filter((l) => l.stage === "Won").length /
    leads.filter((l) => ["Won", "Lost"].includes(l.stage)).length) * 100
);
...
const overrunProjects = projects.filter((p) => p.actual / p.budget > p.progress / 100 + 0.1);
...
export const kpis = {
  ...
  netForecast: cashFlow[cashFlow.length - 1].net,
...
export const projectProfitability = [...projects]
  .map((p) => ({
    ...p,
    profit: Math.round(p.budget * (p.margin / 100)),
    spent: Math.round((p.actual / p.budget) * 100),
  }))
